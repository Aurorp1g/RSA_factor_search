...
try:
    from gmpy2 import gcd as math_gcd
except ImportError:
    try:
        from Crypto.Util.number import gcd as math_gcd
    except ImportError:
        def math_gcd(a, b):
            while b:
                a, b = b, a % b
...
def small_search(square_x, square_y, e, pb, pu, g, N):
    ...
    if x_min <= x0 <= x_max and y_lower <= y0 <= y_upper:
        ...
        if (d := math_gcd(h, N)) > 1:
            return d
    if (d := math_gcd(x0, N)) > 1:
        return d
    if (d := math_gcd(y0, N)) > 1:
        return d
    ...
    for _ in range(max_steps):
        ...
        # 三重因子检查
        if (d := math_gcd(x, N)) > 1 or (d := math_gcd(y, N)) > 1 or (d := math_gcd(h, N)) > 1:
            return d
...
def rsa_factor_search(N, g, eta):
    ...
    Cx = (N - pu + 1) // 2
    Cy = g * (N + pb + 1) // 4 + 1
    if (res := math_gcd(N, pu + Cx - ((Cy - 1)//g) - 1)) > 1:
        return res
    ...
    batch_size = max(1, (R-1) // (max(cpu_count()-1, 1)*2))
...
if __name__ == "__main__":
    ...
    if g == 0 or eta == 0:
        ...
        sys.exit(0 if result else 1)  # 改用正常退出